...
const state = {
  ...
  savings: loadFromStorage(STORAGE_KEYS.savings, [])
};
...
const elements = {
  ...
  savingsAmountInput: document.querySelector("#savingsAmountInput"),
  savingsDateInput: document.querySelector("#savingsDateInput"),
  savingsAmountError: document.querySelector("#savingsAmountError"),
  savingsDateError: document.querySelector("#savingsDateError"),
  savingsMessage: document.querySelector("#savingsMessage"),
...
function calculateSummary(filterFn) {
  ...
  summary.savings = calculateSavingsTotal(filterFn);
  return summary;
}

// Adds up savings records that match a dashboard period such as today, this week, or this month.
function calculateSavingsTotal(filterFn) {
  return state.savings.reduce((total, saving) => {
    const savingsDate = parseLocalDate(saving.date);
    return savingsDate && filterFn(savingsDate) ? total + saving.amount : total;
  }, 0);
}
...
function updateSummaryCard(period, summary) {
  ...
  elements[`${period}Breakdown`].textContent = `Income ${formatCurrency(summary.income)} - Expense ${formatCurrency(summary.expense)} - Savings ${formatCurrency(summary.savings)}`;
}
...
// Renders the savings history list so users can review saved amounts after adding them.
function renderSavings() {
  const savings = [...state.savings].sort((a, b) => b.date.localeCompare(a.date) || b.createdAt - a.createdAt);
  elements.savingsCount.textContent = `${savings.length} ${savings.length === 1 ? "item" : "items"}`;

  if (!savings.length) {
    elements.savingsList.innerHTML = '<p class="empty-state">No savings recorded yet.</p>';
    return;
  }

  elements.savingsList.innerHTML = savings.map(saving => {
    const date = parseLocalDate(saving.date).toLocaleDateString("en-US", {
      month: "short",
      day: "numeric",
      year: "numeric"
    });
    ...
    return `
      <article class="savings-item">
        <div>
          <p class="transaction-title">Saved</p>
          <p class="transaction-meta">${date}</p>
        </div>
        <div class="amount savings">+${formatCurrency(saving.amount)}</div>
      </article>
    `;
  }).join("");
}
...
// Checks the savings form for a valid amount and date before a savings record is stored.
function validateSavingsForm() {
  elements.savingsAmountError.textContent = "";
  elements.savingsDateError.textContent = "";
  const amount = Number(elements.savingsAmountInput.value);
  const date = elements.savingsDateInput.value;
  let valid = true;

  if (!Number.isFinite(amount) || amount <= 0 || amount > MAX_AMOUNT) {
    elements.savingsAmountError.textContent = "Enter an amount from 0.01 to 999,999.";
    valid = false;
  }

  if (!parseLocalDate(date)) {
    elements.savingsDateError.textContent = "Enter a valid date.";
    valid = false;
  }

  return valid ? { amount: roundMoney(amount), date } : null;
}
...
elements.savingsForm.addEventListener("submit", event => {
  event.preventDefault();
  const saving = validateSavingsForm();
  if (!saving) {
  ...
  elements.savingsDateInput.value = toDateInputValue(new Date());
  elements.savingsMessage.textContent = "Savings saved.";
  renderAll();
});

elements.clearAllButton.addEventListener("click", () => {
  const confirmed = window.confirm("Clear every transaction, budget limit, and savings record?");
  if (!confirmed) {
    return;
  }
